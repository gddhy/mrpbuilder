...
char* ex_coding(char* text,int len,char* coding,char* tocoding){
	...
	if(mrc_strcmp(tocoding, "unicode")==0 || mrc_strcmp(tocoding,"UNICODE")==0 || mrc_strcmp(tocoding,"UTF16-BE")==0 || mrc_strcmp(tocoding,"utf16-be")==0){
...
char *unToGb(char *text)
{
 char *input= text;
  int output_len;
int input_len= mrc_wstrlen(input);
 char *output= NULL; //malloc(input_len+2);
 mrc_printf("input_len %d\n",input_len);

//  u2c( input, input_len, &output, &output_len);
 output = UCS2BEStrToGBStr((uint16 *)input,&output_len);
 return output;
}
